...
juan=[]
def CODE():
    global INPUTLARGE;global LABELLARGE
    nami0.set(datos[0])
    nami1.set(datos[1])
    nami2.set(datos[2])
    nami3.set(datos[3])
    nami4.set(datos[4])
    nami5.set(datos[5])
    INPUTLARGE.grid_forget()
    LABELLARGE.grid_forget()
...
for fila in range(0,len(preguntas)):
    col=1
    if (fila==0):
        INPUTX=Entry(FRAME,textvariable=nami0)
    elif (fila==1):                                                                     
        INPUTX=Entry(FRAME,textvariable=nami1)
    elif (fila==2):
        INPUTX=Entry(FRAME,textvariable=nami2)
    elif (fila==3):
        INPUTX=Entry(FRAME,textvariable=nami3)
    elif (fila==4):
        INPUTX=Entry(FRAME,textvariable=nami4)    
